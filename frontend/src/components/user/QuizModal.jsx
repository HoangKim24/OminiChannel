...
const QuizModal = () => {
  ...
  const handleQuizAnswer = (questionId, value) => {
    ...
    if (quizStep < quizQuestions.length - 1) {
      ...
      // Calculate matching
      const scored = normalizedProducts.map(p => {
        let score = 0;
      ...
      const topMatches = scored.sort((a, b) => b.matchScore - a.matchScore).slice(0, 4);
  ...
  return (
    ...
    >
      <div className="quiz-modal" onClick={e => e.stopPropagation()} style={{ position: 'relative', background: '#111', border: '1px solid var(--glass-border)', padding: '3rem', borderRadius: '12px', textAlign: 'center', width: '90%', maxWidth: '800px', margin: 'auto', maxHeight: '90vh', overflowY: 'auto' }}>
        <button type="button" className="icon-btn" style={{ position: 'absolute', top: '10px', right: '10px', fontSize: '1.5rem' }} onClick={closeQuiz}>✕</button>
        {quizStep < quizQuestions.length ? (
          <div key={quizStep} className="fade-in">
            ...
            <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
              {quizQuestions[quizStep].options.map(([val, label]) => (
                <button key={val} type="button" className="btn-gold" style={{ background: 'transparent', border: '1px solid #333', color: '#fff' }}
          ...
          <div className="fade-in">
            ...
            {quizResults.length === 0 && (
              <div style={{ color: '#ccc', marginBottom: '1.5rem' }}>Chưa có kết quả phù hợp từ dữ liệu hiện tại.</div>
            )}
            <div className="product-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '2rem' }}>
              {quizResults.map(p => (
                <div key={p.id} className="product-card" style={{ padding: '0.5rem' }}>
                  <div className="product-image-container" onClick={() => { navigate(`/product/${p.id}`); closeQuiz(); }} style={{ aspectRatio: '1/1', marginBottom: '1rem', cursor: 'pointer' }}>
                    <img src={p.imageUrl} alt={p.name} className="product-image" />
                  </div>
                  <h4 className="brand-font">{p.name}</h4>
                  <p style={{ color: 'var(--accent-gold)', fontSize: '0.9rem' }}>{p.matchScore}% Phù Hợp</p>
                  <button type="button" className="btn-gold" style={{ width: '100%', padding: '0.6rem', marginTop: '1rem', fontSize: '0.75rem' }} onClick={() => { navigate(`/product/${p.id}`); closeQuiz(); }}>Chi Tiết</button>
                </div>
              ))}
            </div>
            <button type="button" className="btn-gold" style={{ marginTop: '4rem' }} onClick={resetQuiz}>Làm Lại Trắc Nghiệm</button>
          </div>
        )}
      </div>
    </div>
